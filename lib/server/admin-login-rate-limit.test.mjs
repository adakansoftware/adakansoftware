...
test("uses shared rate limiting when production database and signing secret are configured", () => {
  assert.equal(shouldUseSharedAdminRateLimit({ NODE_ENV: "production", DATABASE_URL: "postgres://example", ADMIN_SESSION_SECRET: "s".repeat(32) }), true)
  assert.equal(shouldUseSharedAdminRateLimit({ NODE_ENV: "production", DATABASE_URL: "postgres://example" }), false)
  assert.equal(shouldUseSharedAdminRateLimit({ NODE_ENV: "development", DATABASE_URL: "postgres://example", ADMIN_SESSION_SECRET: "s".repeat(32) }), false)
})
