...
test("uses shared rate limiting only when production Redis is configured", () => {
  assert.equal(shouldUseSharedAdminRateLimit({ NODE_ENV: "production", CONTACT_STATE_BACKEND: "file" }), false)
  assert.equal(shouldUseSharedAdminRateLimit({ NODE_ENV: "production", CONTACT_STATE_BACKEND: "redis" }), false)
  assert.equal(shouldUseSharedAdminRateLimit({ NODE_ENV: "production", CONTACT_STATE_BACKEND: "redis", REDIS_URL: "rediss://example.com" }), true)
})
